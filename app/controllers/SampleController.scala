...
@Singleton
class SampleController @Inject()(admindao: adminDao, projectdao: projectDao, sampledao: sampleDao,
  ...
  def uploadData(proname: String): Action[AnyContent] = Action { implicit request =>
  ...
  def sampleManage(proname: String) = Action { implicit request =>
    Ok(views.html.fileupload.data(proname))
  }
  ...
  def toDate(proname: String) = Action { implicit request =>
    Redirect(routes.SampleController.sampleManage(proname))
  }
